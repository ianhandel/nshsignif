...
#' @param codes Codes to return - a names vector - in form c(sig = "code for significance", nonsig = "code for non-significant", nontest = "code for not tested or missing").
#' @return Character vector of "S" (for significant), "NS" (not significant)
#' and "not-tested"
#'
#' @export
parse_pvalue <- function(pvals, alpha = 0.05, codes = c(
                           sig = "S",
                           nonsig = "NS",
                           nontest = "not-tested"
                         )) {
  ...
  conclusions <- dplyr::case_when(
    stringr::str_detect(pvals, "<") & pval_num <= alpha ~ "sig",
    !stringr::str_detect(pvals, ">") & pval_num < alpha ~ "sig",
    pval_num >= alpha ~ "nonsig",
    is.na(pval_num) ~ "nontest"
  )

  unname(codes[conclusions])
}
...
make_studyid <- function(bridge, author, year) {
  ...
  df <- dplyr::mutate(df, author_year = dplyr::if_else(
    !is.na(author) & !is.na(year),
